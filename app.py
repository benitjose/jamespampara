    ...
    def execute(self, query, args=()):
        if self.use_postgres:
            with self.conn.cursor() as cur:
                cur.execute(_prepare_query(query, True), args)
                return cur
        return self.conn.execute(_prepare_query(query, False), args)
...
def get_db():
    if 'db' not in g:
        if not POSTGRES_AVAILABLE:
            raise RuntimeError('PostgreSQL driver is not available in the current environment.')
        if not _is_postgres_url(app.config['DATABASE_URL']):
            raise RuntimeError('The configured database URL is not a valid PostgreSQL URL.')
        try:
            print(f"[db] Using PostgreSQL backend for {app.config['DATABASE_URL']}")
            print("POSTGRES_AVAILABLE =", POSTGRES_AVAILABLE)
            print("DATABASE_URL =", app.config["DATABASE_URL"])
            g.db = DatabaseConnection(
    psycopg2.connect(app.config["DATABASE_URL"], sslmode="require"),
    True
)
        except Exception as exc:
            raise RuntimeError(f'Unable to connect to PostgreSQL: {exc}') from exc
    return g.db
...
def init_db():
    ...
    with db.cursor() as cur:
        ...
        for column_name, column_def in [('class', 'TEXT'), ('school', 'TEXT'), ('pin_code', 'TEXT')]:
            try:
                quoted_name = f'"{column_name}"' if column_name == 'class' else column_name
                cur.execute(f'ALTER TABLE users ADD COLUMN IF NOT EXISTS {quoted_name} {column_def}')
        ...
        if existing_admin is None:
            cur.execute(
                'INSERT INTO users (name, email, password_hash, is_admin, dob) VALUES (%s, %s, %s, %s, %s)',
...
def query_db(query, args=(), one=False):
    if get_db().use_postgres:
        with get_db().cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(_prepare_query(query, True), args)
            rv = cur.fetchall()
    else:
        cur = get_db().execute(_prepare_query(query, False), args)
        rv = cur.fetchall()
